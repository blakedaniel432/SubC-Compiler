...
public class VariableParser extends StatementParser {
	 ...
	 *            the parent parser.
	 */
	public VariableParser(SubCParserTD parent) {
		super(parent);
	}

	private static final EnumSet<SubCTokenType> SUBSCRIPT_FIELD_START_SET = EnumSet.of(LEFT_BRACKET);
	...
	public ICodeNode parse(Token token, SymTabEntry variableId) throws Exception {
		...
		if (!isFunctionTarget) {
			...
			while (SUBSCRIPT_FIELD_START_SET.contains(token.getType())) {
				ICodeNode subFldNode = token.getType() == LEFT_BRACKET ? parseSubscripts(variableType)
	...
	private static final EnumSet<SubCTokenType> RIGHT_BRACKET_SET = EnumSet.of(RIGHT_BRACKET, EQUALS, SEMICOLON);
	...
	private ICodeNode parseField(TypeSpec variableType) throws Exception {
		...
		if ((tokenType == IDENTIFIER) && (variableForm == RECORD)) {
			...
			String fieldName = token.getText(); //REMOVED .toLowerCase()
