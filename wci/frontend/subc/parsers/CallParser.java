...
public class CallParser extends StatementParser {
	 ...
	 *            the parent parser.
	 */
	public CallParser(SubCParserTD parent) {
		super(parent);
	}
	...
	private static final EnumSet<SubCTokenType> COMMA_SET = ExpressionParser.EXPR_START_SET.clone();
	static {
		COMMA_SET.add(COMMA);
	 ...
	 *            the current token.
	 * @param pfId
	 *            the symbol table entry of the procedure or function name.
	 * @param isDeclared
	 *            true if parsing actual parms of a declared routine.
	 * @param isReadReadln
	 *            true if parsing actual parms of read or readln.
	 * @param isWriteWriteln
	 *            true if parsing actual parms of write or writeln.
	 * @return the PARAMETERS node, or null if there are no actual parameters.
	 * @throws Exception
	 *             if an error occurred.
	 */
	protected ICodeNode parseActualParameters(Token token, SymTabEntry pfId, boolean isDeclared, boolean isReadReadln,
		...
		while (token.getType() != RIGHT_PAREN) {
			...
			// scalar, boolean, or a SubC string. Parse any field width and
			// precision.
				...
				if (!((form == SCALAR) || (type == Predefined.booleanType) || (type.isSubCString()))) {
