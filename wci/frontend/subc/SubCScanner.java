...
public class SubCScanner extends Scanner {
	...
	protected Token extractToken() throws Exception {
		...
		} else if (SubCTokenType.SPECIAL_SYMBOLS.containsKey(Character.toString(currentChar))
				|| (currentChar == '&' && peekChar() == '&') || (currentChar == '|' && peekChar() == '|')) {
			token = new SubCSpecialSymbolToken(source);
		} else {
			token = new SubCErrorToken(source, INVALID_CHARACTER, Character.toString(currentChar));
