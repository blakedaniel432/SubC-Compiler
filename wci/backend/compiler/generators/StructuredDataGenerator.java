...
public class StructuredDataGenerator extends CodeGenerator {
	...
	/**
	 * Generate code to allocate the string, array, and record variables of a
		...
		for (SymTabEntry id : ids) {
			if (id.getDefinition() == VARIABLE) {
				...
				if (isStructured(idType)) {
					...
					if (idType.isSubCString() || (idForm == RECORD)) {
	 ...
	 * 
	 * @param variableId
	 *            the variable's symbol table entry.
	 * @param type
	 ...
	 *            the array type.
	 */
	private void generateAllocateArray(SymTabEntry variableId, TypeSpec arrayType) {
		...
		} while ((elmtType.getForm() == ARRAY) && (!elmtType.isSubCString()));
								...
								: elmtType == Predefined.charType ? "char"
										: elmtForm == ENUMERATION ? "int"
												: elmtForm == RECORD ? "java/util/HashMap"
														: elmtType.isSubCString() ? "java/lang/StringBuilder" : null;

		// One-dimensional array.
		if (dimCount == 1) {
			Instruction newArray = isStructured(elmtType) ? ANEWARRAY : NEWARRAY;
			emit(newArray, typeName);
		}

		// Multidimensional array.
		else {
			emit(MULTIANEWARRAY, typeDescriptor(variableId.getTypeSpec()), Integer.toString(dimCount));
		}
	...
	/**
	 * Generate code that loops over the array dimensions to allocate data for each
	 * element.
	 * 
	 * @param variableId
	 *            the symbol table entry of the variable.
	 ...
	 *            the first dimension is 1, second is 2, etc.
	 */
	private void generateAllocateElements(SymTabEntry variableId, TypeSpec elmtType, int dimensionIndex) {
		...
		if (elmtType.isSubCString() || (elmtType.getForm() == RECORD)) {
			emitLoadLocal(Predefined.integerType, tempIndex);
	 ...
	 */
	private void generateAllocateRecord(SymTabEntry variableId, TypeSpec recordType) {
	 ...
	 * 
	 * @param symTab
	 *            the record's symbol table containing fields.
	 */
	 ...
	 * 
	 * @param variableId
	 *            the variable's symbol table entry, or null to store into an array
	 *            element.
	 */
