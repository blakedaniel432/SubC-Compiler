...
public class ExpressionGenerator extends StatementGenerator {
	...
	private void generateBinaryOperator(ICodeNode node, ICodeNodeTypeImpl nodeType) {
		...
		boolean integerMode = TypeChecker.areBothInteger(type1, type2)
				|| (type1.getForm() == ENUMERATION) | (type2.getForm() == ENUMERATION);
		boolean realMode = TypeChecker.isAtLeastOneReal(type1, type2) || (nodeType == DIVIDE);
		boolean characterMode = TypeChecker.isChar(type1) && TypeChecker.isChar(type2);
		boolean stringMode = type1.isSubCString() && type2.isSubCString();
