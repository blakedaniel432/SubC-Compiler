...
public class StatementGenerator extends CodeGenerator {
	...
	/**
	 * Generate code for a statement. To be overridden by the specialized statement
	 * executor subclasses.
	 * 
	 * @param node
	...
	public void generate(ICodeNode node) throws SubCCompilerException {
		...
		if (localStack.getSize() != 0) {
			throw new SubCCompilerException(
					String.format("Stack size error: size = %d after line %d", localStack.getSize(), line));
		}
	}
