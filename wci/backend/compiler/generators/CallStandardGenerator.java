...
public class CallStandardGenerator extends CallGenerator {
	...
	private void generateReadReadln(ICodeNode callNode, RoutineCode routineCode) {
		ICodeNode parmsNode = callNode.getChildren().size() > 0 ? callNode.getChildren().get(0) : null;
		...
		if (parmsNode != null) {
			...
			for (ICodeNode actualNode : actuals) {
				...
				// Generate code to call the appropriate SubCTextIn method.
				emit(GETSTATIC, standardInName, "LSubCTextIn;");
				if (baseType == Predefined.integerType) {
					emit(INVOKEVIRTUAL, "SubCTextIn.readInteger()I");
				} else if (baseType == Predefined.realType) {
					emit(INVOKEVIRTUAL, "SubCTextIn.readReal()F");
				} else if (baseType == Predefined.booleanType) {
					emit(INVOKEVIRTUAL, "SubCTextIn.readBoolean()Z");
				} else if (baseType == Predefined.charType) {
					emit(INVOKEVIRTUAL, "SubCTextIn.readChar()C");
				}
		...
		if (routineCode == READLN) {
			emit(GETSTATIC, standardInName, "LSubCTextIn;");
			emit(INVOKEVIRTUAL, "SubCTextIn.nextLine()V");
	...
	private void generateEofEoln(ICodeNode callNode, RoutineCode routineCode) {
		...
		emit(GETSTATIC, standardInName, "LSubCTextIn;");
		if (routineCode == EOLN) {
			emit(INVOKEVIRTUAL, "SubCTextIn.atEoln()Z");
		} else {
			emit(INVOKEVIRTUAL, "SubCTextIn.atEof()Z");
		}
		...
		if (parmsNode != null) {
			...
			for (ICodeNode writeParmNode : actuals) {
				...
				// Create and append the appropriate format specification.
					...
					String typeCode = dataType.isSubCString() ? "s"
			...
			if (exprCount > 0) {
				...
				for (ICodeNode writeParmNode : actuals) {
					...
					if (nodeType != STRING_CONSTANT) {
						...
						String signature = dataType.getForm() == SCALAR ? valueOfSignature(dataType) : null;
