...
public class CodeGenerator extends Backend {
	...
	/**
	 * Generate code for a statement. To be overridden by the code generator
	 * subclasses.
	 * 
	 * @param node
	...
	/**
	 * Generate code for a routine. To be overridden by the code generator
	 * subclasses.
	 * 
	 * @param routineId
	 ...
	 *            the operation code.
	 * @param operand1
	 *            the text of the first operand.
	 * @param operand2
	 *            the text of the second operand.
	 */
	 ...
	 * @param index
	 *            the variable's index into the local variables array.
	 */
	...
	/**
	 * Emit code to load the value of a variable, which can be a program variable, a
	 * local variable, or a VAR parameter.
	 * 
	 * @param variableId
	...
	protected void emitLoadVariable(SymTabEntry variableId) {
		...
		// Wrapped variable.
			...
			emit(GETFIELD, varParmWrapper(variableType) + "/value", typeDescriptor(variableType));
		}
	 ...
	 * 
	 * @param elmtType
	 *            the element type if character, else null.
	 */
			...
			emit(elmtType == Predefined.integerType ? IALOAD
					: elmtType == Predefined.realType ? FALOAD
							: elmtType == Predefined.booleanType ? BALOAD
									: elmtType == Predefined.charType ? CALOAD : form == ENUMERATION ? IALOAD : AALOAD);
		}
	}
					...
					: elmtType == Predefined.realType ? FASTORE
							: elmtType == Predefined.booleanType ? BASTORE
									: elmtType == Predefined.charType ? CASTORE
											: form == ENUMERATION ? IASTORE : AASTORE);
		}
	}
	...
	protected boolean isStructured(TypeSpec type) {
		...
		return type.isSubCString() || (form == ARRAY) || (form == RECORD);
	}
	 ...
	 * 
	 * @param id
	 *            the symbol table entry of an identifier.
	...
	protected String typeDescriptor(TypeSpec type) {
		...
		while ((form == ARRAY) && !type.isSubCString()) {
			buffer.append("[");
	...
	protected String valueSignature(TypeSpec type) {
		...
		String typeName = type == Predefined.integerType ? "int"
				: type == Predefined.realType ? "float"
						: type == Predefined.booleanType ? "boolean" : type == Predefined.charType ? "char" : "int";

		return (String.format("%s.%sValue()%s", javaType, typeName, typeCode));
	}

	/**
	 * Generate the name of the wrapper to use to pass an actual parameter by
	 * reference.
	 * 
	 * @param type
	 *            the parameter type..
	...
	protected String varParmWrapper(TypeSpec type) {
		...
		return type == Predefined.integerType ? "IWrap"
				: type == Predefined.realType ? "RWrap"
						: type == Predefined.booleanType ? "BWrap" : form == ENUMERATION ? "IWrap" : "CWrap";
	}
}
