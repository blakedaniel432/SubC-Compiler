...
public class StatementExecutor extends Executor {
	...
	/**
	 * Execute a statement. To be overridden by the specialized statement executor
	 * subclasses.
	 * 
	 * @param node
	...
	/**
	 * Convert a Java string to a SubC string or character.
	 * 
	 * @param targetType
	 *            the target type specification.
	 * @param javaValue
	 *            the Java string.
	 * @return the SubC string or character.
	 */
	protected Object toSubC(TypeSpec targetType, Object javaValue) {
		if (javaValue instanceof String) {
			String string = (String) javaValue;

			if (targetType == Predefined.charType) {
				return string.charAt(0); // SubC character
			} else if (targetType.isSubCString()) {
				...
				return charCells; // SubC string (array of characters)
			} else {
				return javaValue;
			}
		} else {
	...
	/**
	 * Convert a SubC string to a Java string.
	 * 
	 * @param targetType
	 *            the target type specification
	 * @param subcValue
	 ...
	 */
	protected Object toJava(TypeSpec targetType, Object subcValue) {
		if ((subcValue instanceof Cell[]) && (((Cell[]) subcValue)[0].getValue() instanceof Character)) {
			Cell charCells[] = (Cell[]) subcValue;
	...
	/**
	 * Return a copy of a SubC record.
	 * 
	 * @param value
	 *            the record value hashmap.
	 * @param node
	 *            the statement node.
	 ...
	 * 
	 * @param node
	 *            the root node of the expression subtree to check.
	 * @param type
	 *            the target type specification.
	 * @param value
	 *            the value.
