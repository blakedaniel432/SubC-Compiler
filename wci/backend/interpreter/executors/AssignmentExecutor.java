...
public class AssignmentExecutor extends StatementExecutor {
	 ...
	 * 
	 * @param node
	 *            the ancester parse tree node of the assignment.
	 * @param targetId
	 *            the symbol table entry of the target variable or parm.
	 * @param targetCell
	 *            the target cell.
	 * @param targetType
	 *            the target type.
	 ...
	 *            the value type.
	 */
	protected void assignValue(ICodeNode node, SymTabEntry targetId, Cell targetCell, TypeSpec targetType, Object value,
			TypeSpec valueType) {
		// Range check.
		...
		// target length > value length: blank pad the value
			...
			targetCell.setValue(copyOf(toSubC(targetType, stringValue), node));
		}

		// Simple assignment.
		else {
			targetCell.setValue(copyOf(toSubC(targetType, value), node));
		}
